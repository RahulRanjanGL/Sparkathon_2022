...
public class IrctcService {
	...
	public TrainInfo getTicketBookingResult(TicketBooking ticketBooking) throws ApplicationException {
		...
		TrainInfo info1 = new TrainInfo();
		
		Iterator<TrainInfo> it = buildTrains().iterator();
		while(it.hasNext()) {
			info = it.next();
			
			if(info.getFrom().contains(ticketBooking.getFromCity()) &&  info.getTo().contains(ticketBooking.getToCity())
					&& info.getTavelClass().contains(ticketBooking.getTravelClass())) { 
				info1 = info;
			}
		}
	...
	public String getPNRNumber(TicketBooking ticketBooking) throws ApplicationException {
		...
		String res = ticketBooking.getMobileNumber().substring(0, 7);
